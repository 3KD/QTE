...
class EntropyTab(ttk.Frame):
    ...
    def compute_certificate(self):
        """Return a Z/QFT entropy certificate for current amplitudes.
    
        Expects `self.last_amplitudes` (1D complex array). Falls back to |0>.
        """
        import numpy as np
        try:
            from entropy_lab import entropy_certificate_pack
        except Exception:
            return {"error": "entropy_lab not available"}
        a = getattr(self, "last_amplitudes", None)
        if a is None:
            a = np.array([1, 0], dtype=complex)
        return entropy_certificate_pack(a)
    """
